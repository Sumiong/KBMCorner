...
export function EnhancedAdminDashboard({ user, onLogout }: EnhancedAdminDashboardProps) {
  ...
  const handleResetLevel = async (userId: string, userName: string, userEmail: string) => {
    ...
    try {
      const profile = getAllUsers().find(u => u.id === userId);
      if (profile) {
        // Reset to Level 1
  ...
  const handleProcessActivation = async () => {
    ...
    try {
      ...
      saveUserProfile(activatingStudent.id, userProfile);
      
      // Also update individual localStorage keys
      localStorage.setItem(`user_membership_level_${activatingStudent.id}`, activationLevel.toString());
      localStorage.setItem(`user_membership_expiry_${activatingStudent.id}`, expiryDateString);
